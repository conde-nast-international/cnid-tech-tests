...
describe('ListingPageItem', () => {
    describe('@render', () => {
        test('has link', () => {
            const item = shallow(<ListingPageItem item={article}/>);
            expect(item.find('.ListingPageItem-link')).toHaveLength(1);
            expect(item.find('.ListingPageItem-link').props().to).toEqual('/article/0');
        });

        test('has image', () => {
            const item = shallow(<ListingPageItem item={article}/>);
            expect(item.find('.ListingPageItem-image')).toHaveLength(1);
            expect(item.find('.ListingPageItem-image').props().src).toEqual(article.cover);
        });

        test('has title', () => {
            const item = shallow(<ListingPageItem item={article}/>);
            expect(item.find('.ListingPageItem-title')).toHaveLength(1);
            expect(item.find('.ListingPageItem-title').text()).toEqual(article.title);
        });
    });
});
